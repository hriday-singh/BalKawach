...
import { LayoutDashboard, Users, Scale, Bell, Settings } from 'lucide-react';
  ...
  return (
    <nav className={styles.bottomNav}>
      ...
      {['system_admin', 'cwc_chairperson', 'cwc_member'].includes(user?.role) && (
        <NavLink to="/hearings" className={({ isActive }) => isActive ? `${styles.navItem} ${styles.active}` : styles.navItem}>
          <Scale size={24} />
          <span className={styles.label}>Hearings</span>
        </NavLink>
      )}
